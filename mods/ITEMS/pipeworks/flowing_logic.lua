...
pipeworks.spigot_check = function(pos, node)
	...
	if belowname and (belowname == "air" or belowname == "base:water_flowing" or belowname == "base:water_source") then 
		...
		if near_node and string.find(near_node.name, "_loaded") then
			if spigotname and spigotname == "pipeworks:spigot" then
				...
				if finitewater or belowname ~= "base:water_source" then
					minetest.add_node({x=pos.x,y=pos.y-1,z=pos.z},{name = "base:water_source"})
				end
			end
		else
			if spigotname == "pipeworks:spigot_pouring" then
				minetest.add_node({x=pos.x,y=pos.y,z=pos.z},{name = "pipeworks:spigot", param2 = fdir})
				if belowname == "base:water_source" and not finitewater then
...
pipeworks.fountainhead_check = function(pos, node)
	...
	if abovename and (abovename == "air" or abovename == "base:water_flowing" or abovename == "base:water_source") then 
		...
		if near_node and string.find(near_node.name, "_loaded") then
			if fountainhead_name and fountainhead_name == "pipeworks:fountainhead" then
				...
				if finitewater or abovename ~= "base:water_source" then
					minetest.add_node({x=pos.x,y=pos.y+1,z=pos.z},{name = "base:water_source"})
				end
			end
		else
			if fountainhead_name == "pipeworks:fountainhead_pouring" then
				...
				if abovename == "base:water_source" and not finitewater then
					minetest.remove_node({x=pos.x,y=pos.y+1,z=pos.z})
				end
			end
		end
