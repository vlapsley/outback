...
-- Dependencies: 			base, farming, biome_lib
-- Supports:
-----------------------------------------------------------------------------------------------
abstract_dryplants = {}
...
local function sickle_on_use(itemstack, user, pointed_thing, uses)
	...
	if minetest.get_item_group(under.name, "snappy") > 0 then
		...
		return itemstack
	elseif string.find(under.name, "base:dirt_with_grass") then
...
minetest.register_node("dryplants:grass_short", {
	...
	tiles = {"base_grass.png^dryplants_grass_short.png", "base_dirt.png", "base_dirt.png^base_grass_side.png^dryplants_grass_short_side.png"},
