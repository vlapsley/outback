				...
				if not (biome.avoid_nodes and biome.avoid_radius and minetest.find_node_near(p_top, biome.avoid_radius + math.random(-1.5,2), biome.avoid_nodes))
				  ...
				  then
					...
					if biome.alt_wallnode and walldir then
						...
						if currentsurface ~= "core:water_source"
						  or (currentsurface == "core:water_source" and #(minetest.find_nodes_in_area({x=pos.x, y=pos.y-biome.depth_max-1, z=pos.z}, {x=pos.x, y=pos.y, z=pos.z}, {"core:dirt", "core:dirt_with_grass", "core:sand"})) > 0 )
						  then
...
function biome_lib:grow_plants(opts)
	...
	options.ground_nodes = options.ground_nodes or { "core:dirt_with_grass" }
	options.grow_nodes = options.grow_nodes or { "core:dirt_with_grass" }
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			if (n_top.name == "air" or n_top.name == "core:snow")
