...
minetest.register_on_generated(function(minp, maxp, seed)
	...
	for z = minp.z + math_floor(grid_size / 2), maxp.z, grid_size do
		...
		if (c == c_lava or c == c_lava_flowing) and noise:get3d({x = x, y = z, z = z}) >= 0.4 then
			for xx = math_max(minp.x, x - grid_size), math_min(maxp.x, x + grid_size) do
			for yy = math_max(minp.y, y - grid_size), math_min(maxp.y, y + grid_size) do
			for zz = math_max(minp.z, z - grid_size), math_min(maxp.z, z + grid_size) do
